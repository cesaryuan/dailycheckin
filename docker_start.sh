...
curl https://github.com/sitoi/dailycheckin/raw/main/docker/config.template.json -o config/config.json
...
if [ $? -ne 0 ];then
  ...
  if [ $? -ne 0 ];then
    ...
    curl -O https://github.com/sitoi/dailycheckin/raw/main/docker/docker-compose.yml
    echo "下载 Makefile 文件（可以无视）"
    curl -O https://github.com/sitoi/dailycheckin/raw/main/docker/Makefile
